...
def get_currency():
    return _get_currency_resource(
        'items',
        lambda x: Currency(x['code'], x['amount_to_usd'])
    )


def calculate_in_all_curr(usd_amount):
    print('-- {} USD in all curencies --'.format(usd_amount))
    for currency in get_currency():
        print("In {}: {:.2f}". format(currency.code,
                                      currency.in_currency(usd_amount)))


print(get_currency_codes())
print(get_currency())
calculate_in_all_curr(1000)
